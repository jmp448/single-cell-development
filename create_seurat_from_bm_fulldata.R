...
for(i in 1:3){
  for(j in 1:6){
    ...
    SObject_temp <- CreateSeuratObject(bmrawdat_temp, min.cells=3, min.features=200,project=paste0("CD",i,"col",j))
...
# this filter destroyed a few samples (in terms of cell numbers): CD2col1, CD2col2, CD2col6 especially
# maybe i should rethink how strict it is?
...
for (i in 1:3){
  for (j in 1:6){
    ...
    demux_temp <- read.table(paste0("/project2/gilad/reem/singlecellCM/round1/fulldata/CD",i,"/CD",i,"col",j,"/demux/hpCD",i,"col",j,"_demux.best"),header=T,stringsAsFactors=F)
...
# a range of cell nos per diffday from 10026 in D15 to 33179 in D1
# (lots of NAs: 44468)

all_cols_noNA_S = subset(all_cols_S, idents = c("Day 1","Day 3","Day 5","Day 7","Day 11","Day 15","Day 0"))

# now this object has 26381 features across 150750 samples
...
# and save this RObj so I can get to it again without running all the previous code
# saveRDS(all_cols_noNA_S,file="Sobj_allhp_noNAdays.rds")
# and to load
